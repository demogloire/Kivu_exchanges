...
from ..models import Trucking, Transit
...
def littruck():
   ...
   if current_user.role!='Admin':
      return redirect(url_for('main.dashboard'))
   #Titre
   title='Trucking number | Kivu Exchange'
...
def statutranist(transit_id):
   #Titre
   title='Statut Tracking | Kivu Exchange'
...
def datelivraison(transit_id):
   #Titre
   title='Livarsion colis | Kivu Exchange'
...
def bordtransit(transit_id):
   #Titre
   title='Bordereaux | Kivu Exchange'
...
def trackdocmod(transit_id):
   ...
   form=MofTrucking()
   #Titre
   title='Modifier bordeaux | Kivu Exchange'
   #Requête de vérification du trancking
   track_statu=Trucking.query.filter_by(id=transit_id).first_or_404()
   #Le nom du type encours de modification
