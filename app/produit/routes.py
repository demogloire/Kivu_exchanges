...
def ajoutprod():
   ...
   if form.validate_on_submit():
      ...
      if form.img_url.data=="":
         produit_enre=Produit(nom=nom_pro, categorie_id=form.rech_cate.data.id, prix_p=form.prix_p.data,user_produit=current_user, statut=False, code=code_produit)
         ...
         produit_enre=Produit(nom=nom_pro, categorie_id=form.rech_cate.data.id, prix_p=form.prix_p.data,user_produit=current_user, img_url=form.img_url.data, statut=False, code=code_produit)
         db.session.add(produit_enre)
         db.session.commit()
...
def upload_file():
   ...
   if form.validate_on_submit():
      ...
      if file_to_upload:
         ...
         prod_img.img_url=upload_result['url']
         db.session.commit()
         flash("Ajout d'un nouveau produit",'success')
         session.pop('img',None)
         return redirect(url_for('produit.lipro')) 
